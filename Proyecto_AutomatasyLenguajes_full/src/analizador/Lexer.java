...
/**
 * Simple lexer that recognizes the tag-based pseudocode used in the example.
 * Not generated by JFlex but designed to be robust for the provided grammar.
 */
public class Lexer {
    ...
    private String readLineSafe() throws IOException {
    ...
    private String fill() throws IOException {
        ...
        String lineStr;
        while ((lineStr = readLineSafe()) != null) {
            sb.append(lineStr);
            // stop if we got a tag to make lexing easier (we can still work with full string)
    ...
    private Token fromMatcher(Matcher m, TokenType type, String lexeme) {
        return new Token(type, lexeme, line);
    }

    // Simplified approach: read the whole remaining content into a string and iterate
    ...
    public Token nextToken() throws IOException {
        ensureRemaining();
        if (remaining==null) return new Token(TokenType.EOF, null, line);
        // skip whitespace and newlines while updating line count
        ...
        if (mtag.find()) {
            String closing = mtag.group(1); // "/" if closing
        ...
        // if nothing matched, consume one char as error
